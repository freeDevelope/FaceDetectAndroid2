...
#define info(...) __android_log_print(ANDROID_LOG_INFO, "MaiPai", __VA_ARGS__)
// **************** Java to C++ **************** //
...
/**
 * @author yuyuankun
 * @Description 获取C++对象的人脸矩形框
 * @param _rect java对象的人脸矩形框
 * @return C++对象的人脸矩形框
 */
cv::Rect getNativeRect(JNIEnv *env, jobject _rect)
{
    jclass class_Rect = env->FindClass("org/opencv/core/Rect");//获取class对象
    ...
    jfloat x = env->GetIntField(_rect, field_x);
    jfloat y = env->GetIntField(_rect, field_y);
    jfloat width = env->GetIntField(_rect, field_width);
    jfloat height = env->GetIntField(_rect, field_height);
